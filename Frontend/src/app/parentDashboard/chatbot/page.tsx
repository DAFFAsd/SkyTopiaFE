...
import Image from 'next/image'; // Kita butuh Image buat logo
...
interface Session {
    ...
    const fetchChatHistory = async (threadId: string) => {
        setActiveThreadId(threadId); // Tandain sesi ini aktif
        setIsLoadingHistory(true);
        setMessages([]); // Kosongin chat lama
        try {
        const token = localStorage.getItem('token');
        if (!token) throw new Error('Token tidak ditemukan');
    ...
    const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
        ...
        setNewMessage(''); // Kosongin input box

        // Tambahin pesan kita ke UI duluan
        ...
        if (data.success) {
            ...
            // Kalo ini chat baru, update state & panggil ulang daftar sesi
            if (!activeThreadId) {
            setActiveThreadId(data.thread_id);
            fetchSessions(); // Biar chat barunya muncul di kiri
            }
        } else {
        ...
        // Kalo gagal, balikin pesen user ke input box (opsional)
    ...
    const handleDeleteSession = async (e: React.MouseEvent, threadId: string) => {
        e.stopPropagation(); // Biar nggak nge-trigger 'handleSelectSession'
        ...
        try {
            ...
            // Kalo yang dihapus adalah chat yang lagi aktif, bersihin layar
    ...
    // =======================================================================
    return (
        <div className="space-y-6 flex flex-col h-full"> {/* (BARU) Bikin 'h-full' */}
        <Link
        ...
        {/* --- (BARU) Kontainer Utama Chat (2 kolom) --- */}
        {/* Kita pake 'flex-1' biar dia ngisi sisa layar & 'overflow-hidden' */}
